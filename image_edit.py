...
class ImageEdit:
    ...
    def flip(self, mode):
        self.image = self.image.trancepose(mode)
    ...
    def convert(self, mode):
        if mode == 'roll':
            if self.image.mode != 'RGB':
                raise ValueError(f'Can`t roll with not RGB mode "{self.image.mode}"')
            ...
            if self.image.mode != 'RGB':
                raise ValueError(f'Can`t split channel with not RGB mode "{self.image.mode}"')

            colors_channel = np.array(self.image)
            colors_channel[:, :, (mode != 'R', mode != 'G', mode != 'B')] *= 0
            self.image = Image.fromarray(colors_channel)
            return
